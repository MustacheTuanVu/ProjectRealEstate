 ...
 * @author Cuong
 */
@WebServlet(name = "EstateCreate1", urlPatterns = {"/EstateCreate1"})
public class EstateCreate1 extends HttpServlet {
...
//                    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (estateListByAdress != null) {
            ...
            try {
                estateControl.create(estate);
                response.sendRedirect(request.getContextPath() + "/EstateCreate1?user=employee&modal=show");
            } catch (RollbackFailureException ex) {
                Logger.getLogger(EstateCreate.class.getName()).log(Level.SEVERE, null, ex);
            } catch (Exception ex) {
                Logger.getLogger(EstateCreate.class.getName()).log(Level.SEVERE, null, ex);
